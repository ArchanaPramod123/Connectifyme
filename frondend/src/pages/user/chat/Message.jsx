...
// //   const handleVideoCall = (calleeEmail) => {
// //     if (ws) {
// //         ws.send(JSON.stringify({ type: 'video_call', callee: calleeEmail }));
// //     }
// // };
...
//       ws.send(JSON.stringify({ type: 'video_call', caller: callerEmail, callee: calleeEmail }));
//   }
// };
...
//                   onClick={() => handleVideoCall(message.sender_email)}
//               >
...
//                 // Handle incoming video call, e.g., navigate to the video call page
...
useEffect(() => {
  if (ws) {
      ws.onmessage = (event) => {
          ...
          if (message.type === 'video_call') {
              // Display notification for incoming call
              setVideoCallDetails({
...
useEffect(() => {
  if (videoCallDetails) {
      // Redirect to the video call page with the specific room ID
      if (window.confirm(`Incoming video call from ${videoCallDetails.caller}. Do you want to accept?`)) {
          navigate(`/user/video-call/${videoCallDetails.roomId}`);
      } else {
          setVideoCallDetails(null);  // Reset if declined
      }
  }
}, [videoCallDetails]);
...
const handleVideoCall = (calleeEmail) => {
  if (ws) {
      const callerEmail = email;
      ws.send(JSON.stringify({ type: 'video_call', caller: callerEmail, callee: calleeEmail }));
      // Redirect to the video call page with a unique room ID
        ...
        <div className="flex h-screen  p-2">
          ...
          <div className="flex flex-col flex-grow w-3/5 mt-5 p-1 m-2 bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.27),0_10px_20px_-2px_rgba(0,0,0,0.04)] rounded-lg overflow-hidden">
            ...
            {bg ? (
              ...
              <button
                  className="text-blue-500 hover:text-blue-700"
                  onClick={() => handleVideoCall(dpChat)}
              >
                  <span className="material-symbols-outlined">video_call</span>
              </button>
          </div>
